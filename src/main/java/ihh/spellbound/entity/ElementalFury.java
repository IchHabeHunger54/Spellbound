...
public class ElementalFury extends SpellProjectile {
    ...
    protected void affectEntity(EntityHitResult result) {
        if (!((LivingEntity) result.getEntity()).hasEffect(EffectInit.spell_shield)) {
            if (!((LivingEntity) result.getEntity()).hasEffect(EffectInit.cold_shield)) {
            ...
            if (!((LivingEntity) result.getEntity()).hasEffect(EffectInit.fire_shield))
                result.getEntity().hurt(DamageSource.ON_FIRE, Config.GREATER_FIREBALL_DAMAGE.get());
            if (!((LivingEntity) result.getEntity()).hasEffect(EffectInit.lightning_shield)) {
