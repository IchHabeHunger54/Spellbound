...
public final class AdvanceTime extends Spell {
    ...
    protected boolean use(ItemStack stack, PlayerEntity player, ServerWorld world) {
        ...
        if (Config.ADVANCE_TIME_RANGE.get() > 0)
            for (BlockPos b : Util.getBlocksInRange(world, player.getPosX(), player.getPosY(), player.getPosZ(), Config.ADVANCE_TIME_RANGE.get(), blockState -> blockState instanceof IGrowable))
