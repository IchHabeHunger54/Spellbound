        ...
        if (d == Direction.UP) {
            for (int w = -Config.OBSIDIAN_WALL_HORIZONTAL.get(); w <= Config.OBSIDIAN_WALL_HORIZONTAL.get(); w++)
                for (int h = -Config.OBSIDIAN_WALL_HORIZONTAL.get(); h <= Config.OBSIDIAN_WALL_HORIZONTAL.get(); h++)
                    b = Util.replaceAirBlock(world, new BlockPos(player.getPositionVec().add(w, Config.OBSIDIAN_WALL_VERTICAL.get(), h)), BlockInit.DECAYING_OBSIDIAN.get()) || b;
            ...
            for (int h = 0; h < Config.OBSIDIAN_WALL_VERTICAL.get(); h++) {
                if (d == Direction.SOUTH)
                    b = Util.replaceAirBlock(world, new BlockPos(player.getPositionVec().add(w, h, Config.OBSIDIAN_WALL_VERTICAL.get())), BlockInit.DECAYING_OBSIDIAN.get()) || b;
                if (d == Direction.EAST)
                    b = Util.replaceAirBlock(world, new BlockPos(player.getPositionVec().add(Config.OBSIDIAN_WALL_VERTICAL.get(), h, w)), BlockInit.DECAYING_OBSIDIAN.get()) || b;
