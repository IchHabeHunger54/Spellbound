...
public final class ColorSpray extends Spell {
    ...
    public ColorSpray() {
        super(Config.COLOR_SPRAY_USE_DURATION);
    }

    @Override
    protected boolean use(ItemStack stack, PlayerEntity player, ServerWorld world) {
        boolean b = false;
        for (LivingEntity e : Util.getEntitiesInRange(world, player, Config.COLOR_SPRAY_HORIZONTAL.get(), Config.COLOR_SPRAY_VERTICAL.get()))
        ...
        for (int x = -Config.COLOR_SPRAY_HORIZONTAL.get(); x <= Config.COLOR_SPRAY_HORIZONTAL.get(); x++)
            for (int z = -Config.COLOR_SPRAY_HORIZONTAL.get(); z <= Config.COLOR_SPRAY_HORIZONTAL.get(); z++) {
                BlockPos pos = player.getPosition().add(x, -1, z);
