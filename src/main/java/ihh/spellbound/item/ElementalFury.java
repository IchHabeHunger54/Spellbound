...
public final class ElementalFury extends Spell {
    public ElementalFury() {
        super(Config.ELEMENTAL_FURY_USE_DURATION);
    }

    @Override
    protected boolean use(ItemStack stack, PlayerEntity player, ServerWorld world) {
        for (LivingEntity e : Util.getEntitiesInRange(world, player, Config.ELEMENTAL_FURY_HORIZONTAL.get(), Config.ELEMENTAL_FURY_VERTICAL.get()))
            if (!e.isPotionActive(EffectInit.SPELL_SHIELD.get())) {
                ...
                world.setBlockState(player.getPosition(), Blocks.FIRE.getDefaultState());
